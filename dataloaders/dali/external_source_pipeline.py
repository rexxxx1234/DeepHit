...
class ExternalSourcePipeline(Pipeline):
    def __init__(
        ...
        self.image_cmnp = ops.CropMirrorNormalize(
            device="gpu",
        ...
        self.mask_augmentation = ops.PythonFunction(
            function=base_mask_augmentation(False), num_outputs=1
        )
        self.mask_res = ops.Resize(device="gpu", resize_x=512, resize_y=512)
        self.mask_rotate = ops.Rotate(
            device="gpu", interp_type=types.INTERP_LINEAR, fill_value=1
        )
        self.mask_rng = ops.Uniform(range=(-180.0, 180.0))
        self.mask_cmnp = ops.CropMirrorNormalize(
            device="gpu",
            output_dtype=types.FLOAT,
            output_layout=types.NCHW,
            crop=(512, 512),
